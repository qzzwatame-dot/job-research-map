...
function renderResult(result) {
  ...
  els.shareSummary.textContent = `${result.type.lead} 相性が良い業界は ${result.industries.slice(0, 3).map((d) => d.industry).join("、")}。`;
}
...
async function copyResultText() {
  if (!state.result) return;
  const text = `私のキャリア相性診断は「${state.result.type.label}」。相性が良い業界は ${state.result.industries.slice(0, 3).map((d) => d.industry).join("、")}。おすすめ企業は ${state.result.companies.slice(0, 3).map((d) => d.company.company).join("、")}。`;
  try {
